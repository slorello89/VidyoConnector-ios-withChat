...
- (void)viewDidLoad {
    ...
    // Provide the vc to the app delegate so other tabs can have access to it.
    [(VidyoConnectorAppDelegate *)[[UIApplication sharedApplication] delegate] setVidyoConnector:vc];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    if (textField == host) {
        // when hitting return in host field, switch to token field
        [token becomeFirstResponder];
    } else if (textField == token) {
        // when hitting return in token field, switch to display name field
        [displayName becomeFirstResponder];
    } else if (textField == displayName) {
        // when hitting return in display name field, switch to resource Id field
        [resourceId becomeFirstResponder];
    } else if (textField == resourceId) {
        // when hitting return in resource Id field, hide the keyboard
    ...
    if ([toggleConnectButton imageForState:UIControlStateNormal] == callEndImage) {
        ...
        if (![appSettings vidyoCloudJoin]) {
            ...
            if ( [trimmedResourceId containsString:@" "] || [trimmedResourceId containsString:@"@"] ) {
                ...
                if (![vc connect:[[[host text] stringByTrimmingCharactersInSet: [NSCharacterSet whitespaceCharacterSet]] UTF8String]
            ...
            [self changeState:VidyoConnectorStateConnecting];
            if (![vc connectToRoomAsGuest:[[appSettings portal] UTF8String]
                                  DisplayName:[[[displayName text] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] UTF8String]
                                      RoomKey:[[appSettings roomKey] UTF8String]
                                      RoomPin:[[appSettings roomPin] UTF8String]
                            ConnectorIConnect:self]) {
                // Connect failed.
                [self changeState:VidyoConnectorStateFailure];
            }
...
// Implementation of VCConnectorIRegisterLocalMicrophoneEventListener
...
// Implementation of VCConnectorIRegisterLocalSpeakerEventListener
...
// Implementation of VCConnectorIRegisterLogEventListener
